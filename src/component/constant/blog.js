export const blogsData = [
  {
    cardImage: require("../../assests/images/blog-1.png"),
    topic: "Database",
    title: "Building microBuilding microservices with Dropwizard, MongoDB & Dockerservices with Dropwizard,",
    text: "This NoSQL database oriented to documents (by documents like JSON) combines some of the features from relational databases, easy to use and the multi-platform is the best",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Jan 17, 2022",
      time: "3 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-2.png"),
    topic: "DatabaseWeb Performance",
    title: "Fast web page loading on a $20 feature phone",
    text: "Feature phones are affordable (under $20-25), low-end devices enabling 100s of millions of users in developing countries to leverage the web. Think of them as a light",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Dec 10, 2021",
      time: "2 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-3.png"),
    topic: "Accessibility",
    title: "Accessibility Tips for Web Developers",
    text: "It's awesome to build sites that are inclusive and accessible to everyone. There are at least six key areas of disability we can optimize for: visual, hearing, mobility,",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Nov 28, 2021",
      time: "4 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-4.png"),
    topic: "Database",
    title: "Dynamically Securing Databases using Hashicorp Vault",
    text: "Nowadays, it's hard to profoundly talk about security in the IT industry, since it has to be considered on so many different levels: from securing code chunks, securing",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Nov 20, 2021",
      time: "4 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-5.png"),
    topic: "Web Performance",
    title: "Adaptive Loading - Improving Web Performance on low-end devices",
    text: "Adaptive Loading: Do not just respond based on screen size, adapt based on actual device hardware. Any user can have a slow experience. In a world with widely",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Nov 10, 2021",
      time: "3 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-6.png"),
    topic: "Accessibilty",
    title: "Don't Develop Just for Yourself - A Developer's Checklist to Accessibility",
    text: "We, as developers, tend to develop sites unconsciously for people like ourselves. If we don't actively pay attention, the sites are often accessible only for certain",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Oct 25, 2021",
      time: "7 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-7.png"),
    topic: "Database",
    title: "Building a Restful CRUD API with Node JS, Express, and MongoDB",
    text: "Application Programming Interface is the abbreviation for API. An API is a software interface that enables two apps to communicate with one another. In other words, an API",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Oct 15, 2021",
      time: "5 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-8.png"),
    topic: "Web Performance",
    title: "Monitoring Performances with the PagesSpeed Insights API ",
    text: "The PageSpeed Insights API provides free access to performance monitoring for web pages and returns data with suggestions for how to improve. The V5 API includes",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Oct 3, 2021",
      time: "5 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-9.png"),
    topic: "Accessibility",
    title: "The best web accessibility tools for developers in 2021 ",
    text: "The quality of the tools you use defines the speed with which you can diagnose and resolve problems. Each year the landscape changes dramatically in web technologies,",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Sep 13, 2021",
      time: "7 min",
    }
  },
  {
    cardImage: require("../../assests/images/blog-10.png"),
    topic: "DataBase",
    title: "The MERN (MongoDB, Express, React, NodeJS) stack is very popular for making full stack applications, utilizing Javascript for both the backend and frontend as well as ",
    aurthor: {
      name: "Julia Walker",
      image: require("../../assests/images/author.png"),
      date: "Sep 21, 2021",
      time: "4 min",
    }
  },
]


export const tags = [
"#mongodb",
"#a11y",
"#mobility",
"#inclusion",
"#webperf",
"#optimize",
"#performance"
]

export const cardContent =[
  {
    title:"Let's Talk",
    text:"Do you want to learn more about how I can help your company overcome problems? Let us have a conversation"
  },
  {
    title:"Get in Touch",
    text:"Do you want to learn more about how I can help your company overcome problems? Let us have a conversation"
  }
]




